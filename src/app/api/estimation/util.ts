...
import { ServicePriceInfo, ServiceSummary, UberServiceData } from '../types/uber';

const getPolyLine = async (originLocation: MyGeoPoint, destinationLocation: MyGeoPoint): Promise<{
  distance: number;
  duration: number;
  polyline: string;
}> => {
  const ACCESS_TOKEN = 'pk.eyJ1IjoiY2hyaXNoYXdlcyIsImEiOiJjbGNpbGV0dmc0M21xM29tb2syZHE3YmU4In0.Vse1fk7Oga4WpeJsWIH28w';
  const BASE_URL = 'https://api.mapbox.com/directions/v5/mapbox';
  
  try {
    const response = await axios.get(`${BASE_URL}/driving/${originLocation.longitude},${originLocation.latitude};${destinationLocation.longitude},${destinationLocation.latitude}`, {
      params: {
...
const getPriceEstimation = async (originLocation: MyGeoPoint, destinationLocation: MyGeoPoint, distance: number, duration: number, polyline: string) => {
  const crfToken = 'UnkRXxW9r0lfBZsasyDToZxlbrYgUFIE5YjoV1jbIEHRcR5W4DrP3uFKaNxqZIMv';
  try {
...
export const getEstimation = async (originLocation: MyGeoPoint, destinationLocation: MyGeoPoint) => {
  const { distance, duration, polyline } = await getPolyLine(originLocation, destinationLocation);
  const priceEstimation = await getPriceEstimation(originLocation, destinationLocation, distance, duration, polyline);
  return priceEstimation;
}

function parseRateInfo(notes: string[]): { perMile?: number; perMinute?: number } {
  // Look for the note that contains rate information
  const rateNote = notes.find(note => note.includes('per mile') || note.includes('per minute'));
  if (!rateNote) return {};
